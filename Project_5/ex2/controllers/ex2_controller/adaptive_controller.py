...
class AdaptiveController(BaseController):
    ...
    def initializeGainMatrix(self):
        ...
        B_c[:12,:] = B_p
        Bc_c = np.zeros((16,4))
        Bc_c[12:,:] = -np.identity(4)

        C_c = np.zeros((4,16))
        C_c[:,:12] = C_p
        
        B = B_c
        # B = np.hstack((Bc_c,B_c))
        
        D_c = np.zeros((4,4))
        # A_d,Bc_d,_,_,_ = signal.cont2discrete((A_c,Bc_c,C_c,D_c), self.delT, method='zoh')
        # print(A_d)
        A_d,B_tot_d,C_d,D_d,dt = signal.cont2discrete((A_c,np.hstack((B_c,Bc_c)),C_c,D_c), self.delT, method='zoh')
        # print(A_d)
        
        # A_d,Bc_d,C_d,D_d,dt = signal.cont2discrete((A_c,B_c,C_c,D_c), self.delT, method='zoh')
        # print(A_d)
