...
class AStar(object):
    ...
    def heuristic(self, current, goal):
        ...
        Euclidean distance
        """
        d_euclidean = np.sqrt( (goal[0]-current[0])**2 + (goal[1]-current[1])**2 )
        return d_euclidean
    ...
    def plan(self, start_ind, goal_ind):
        ...
        while len(open_list):
            ...
            for successor in self.get_successor(current):
                ...
                successor.g_value = np.sqrt( (successor.pose[0]-current.pose[0])**2 + (successor.pose[1]-current.pose[1])**2 )
                successor.h_value = self.heuristic(current.pose,successor.pose)
